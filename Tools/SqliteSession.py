...
from Tools.GlobalVars import G_SESSION_DIR
...
class ManualSession:
    def __init__(self, directory=None):
        if directory is None:
            directory = G_SESSION_DIR
        directory = os.path.abspath(directory)
        if not os.path.exists(directory):
            os.mkdir(directory)
            os.chmod(directory, int('700', 8))
        self.directory = directory

    def open_session(self, sid="", create=False):
        if sid is None:
            raise APIException(SystemErrorCode.SessionIdCannotNone, "Session Id Cannot be None!If you want to create a Session Please not pass args!")
        ...
        if 'EXPTIME' not in rv:
            if create:
                ...
                raise APIException(SystemErrorCode.SessionIdInvalid, "Session Invalid!")
        else:
            exp_date = rv['EXPTIME']
            if datetime.datetime.now() - exp_date > datetime.timedelta(days=30):
                self.close_session(rv)
                rv = None
                rv = self.open_session()
        return rv

    def close_session(self, session=None, sid=None):
        if sid is not None:
            try:
