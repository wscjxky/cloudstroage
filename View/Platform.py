...
from Models.CloudStorge import Object, Share, User, PlatformSetting, Domain, File
...
def requirementReply():
    ...
    try:
        group = RequirementReplyGroup.query.filter_by(RequirementId=param[u'Id'], ServiceDomainId=user.DomainId,
                                                      FollowerId=param[u'FollowerId']).one()
    except NoResultFound:
        return []
...
def publishRequirementReply():
    ...
    if session['IsService']:
        ...
        try:
            RequirementFollower.query.filter_by(FollowerDomainId=session['DomainId'],
                                                RequirementId=param[u'RequirementId'], Id=param[u'FollowerId']).one()
        except NoResultFound:
            return {'failed': True}
    else:
    ...
    try:
        group = RequirementReplyGroup.query.filter_by(RequirementId=param[u'RequirementId'],
...
def removeRequirementFollower():
    ...
    if u'Id' in param:
        ...
        if u'Deny' in param:
            ...
            pm = PlatformMsg('取消申请', log, session['UserId'], rf.Follower.Id, param[u'ReqId'], 'requirement',
                             'cancelFocus')
            db.session.add(pm)
        else:
...
def getRequirementFollower():
    ...
    if u'FollowerId' in param:
        if session['IsService']:
            rf = RequirementFollower.query.filter_by(RequirementId=reqId, FollowerDomainId=domainId,
                                                     Id=param[u'FollowerId']).all()
        else:
...
def saveReqScript():
    ...
    param = request.json_param
    try:
        rf = RequirementFollower.query.filter_by(RequirementId=param[u'ReqId'], FollowerDomainId=session['DomainId'],
                                                 Id=param[u'FollowerId']).one()
    except NoResultFound:
        return APIException(SystemErrorCode.UnkonwnError, u'无法修改需求方案，无该制作商的需求申请记录')
...
def applyContract():
    ...
    order = Order(str(uuid4()), '合同押金', '合同押金支付，开启视频制作项目',
                  round_(contract.Amount * contract.DepositPercent / 100), contract.CustomerUserId,
                  contract.ServiceUserId)
...
def editContractUser():
    ...
    if param[u'Operator'] == 'add':
        ...
        if session['IsService']:
            ContractSegment.query.filter_by(ContractId=contractId, ServiceResponser=param[u'UserId']).update(
                {'ServiceResponser': None})
        else:
            ContractSegment.query.filter_by(ContractId=contractId, CustomerResponser=param[u'UserId']).update(
                {'ServiceResponser': None})
            ...
            if u.UserId != session['UserId']:
                try:
                    pm = PlatformMsg.query.filter_by(ResourceId=param[u'ContractId'], ResourceType='contract',
                                                     MsgType='reply', SrcUserId=session['UserId'],
                                                     DestUserId=u.UserId).one()
                    pm.Msg = log
                    pm.IsAdopted = 0
                    pm.IsReaded = 0
                except NoResultFound:
...
def myContract():
    ...
    if u'Id' in param:
        # 清除相关日志
        try:
            PlatformMsg.query.filter_by(ResourceType='contract', ResourceId=param[u'Id'],
                                        DestUserId=session['UserId']).delete()
            db.session.commit()
        except:
            pass
...
def addContractAttachment_p(param, session):
    ...
    groupAUser = set([user.UserId for user in ContractUserGroup.query.filter(ContractUserGroup.DomainId == domainAId,
                                                                             ContractUserGroup.ContractId == param[
                                                                                 u'ContractId']).all()])
    groupBUser = set([user.UserId for user in ContractUserGroup.query.filter(ContractUserGroup.DomainId == domainBId,
                                                                             ContractUserGroup.ContractId == param[
                                                                                 u'ContractId']).all()])
...
def addContractAttachment():
    ...
    pm = PlatformMsg('添加审片素材', log, session['UserId'], c.CustomerUserId, c.RequirementId, 'requirement',
                     'check_attachment')
    db.session.add(pm)
    db.session.commit()
    ca.Id
...
def ContractAttachmentIsPassed():
    ...
    if u'AttachmentId' in param:
        ...
        attachment.Status = 1
        log = '用户驳回了审核视频，请及时查看'
        pm = PlatformMsg('驳回审片素材', log, session['UserId'], attachment.ProviderUserId, attachment.Contract.RequirementId,
                         'requirement', 'check_attachment')
        db.session.add(pm)
    db.session.commit()
    return {u'success': True}
...
def addMarkPoint():
    param = request.json_param
    if not param[u'Time']:
        time = '00:00:' + str(random.randint(10, 99))
        mark = MarkPoint(param[u'AttachmentId'], time, param[u'Content'])
        db.session.add(mark)
    else:
        try:
...
def delMarkPoint():
    param = request.json_param
    try:
        mark = MarkPoint.query.filter_by(Id=param[u'Id']).one()
        print mark
    except NoResultFound:
        return param
...
def editContract():
    ...
    if segDiff > 0:
        ...
        reviewSegs = contract.ContractSegmentDynamic.filter(
            ContractSegment.Segment == ContractSegmentType.Reviewed).all()
...
def confirmContract():
    ...
    if param[u'IsReject']:
        contractHistory = db.session.query(ContractHistory).filter(
            and_(ContractHistory.ContractId == contract.Id, ContractHistory.Version == contract.Version)).one()
        contractHistory.ContractId = contract.Id
        contractHistory.Version = contract.Version
        ...
        if contractSegment.Remark == u'商订合同':
            order = Order(str(uuid4()), '合同押金', '合同押金支付，开启视频制作项目', contract.Amount * contract.DepositPercent / 100,
                          contract.CustomerUserId, contract.ServiceUserId)
            db.session.add(order)
            db.session.flush()
            ...
            order = Order(str(uuid4()), '合同尾款', '合同尾款支付', contract.Amount * (1 - contract.DepositPercent / 100),
                          contract.CustomerUserId, contract.ServiceUserId)
            db.session.add(order)
            db.session.flush()
            log = '需求方\'' + contract.CustomerUser.Domain.CompanyName + '\',制作商\'' + \
    ...
    if param[u'IsReject']:
        title = '环节驳回'
        log = '用户\"' + session['DomainName'] + '\"驳回了您针对合同\"' + contract.Title + '\"的环节确认申请，驳回理由为：\"' + param[
            u'ConfirmRemark'] + '\"'
    else:
        title = '环节通过'
        log = '用户\"' + session['DomainName'] + '\"通过了您针对合同\"' + contract.Title + '\"的环节确认申请，备注信息为：\"' + param[
            u'ConfirmRemark'] + '\"'
...
def aliPay():
    ...
    if order.Amount - order.UseAccount <= 0:
        ...
        url = create_direct_pay_by_user(order.SerialNumber, param['Subject'], param['Description'],
                                        order.Amount - order.UseAccount)
        # 去支付页面
...
def CreateOrder():
    ...
    if order_type == 'RechargeAccount':
        order = Order(str(uuid4()), '充值', '充值:' + str(param[u'RechargeAccount']), float(param[u'RechargeAccount']),
                      session['UserId'], '0')
        db.session.add(order)
        db.session.flush()
...
def getReqTemplate():
    param = request.json_param
    try:
        return RequirementTemplate.query.filter_by(Category_1=param[u'Category_1'],
                                                   Category_2=param[u'Category_2']).one()
    except NoResultFound:
...
def delMsg():
    ...
    if u'UserId' in param:
        ...
        PlatformMsg.query.filter_by(DestUserId=session['UserId'], ResourceId=param[u'RequirementId'],
                                    ResourceType='requirement').delete()
    db.session.commit()
    return {}
...
def applyAbortContract():
    ...
    try:
        ...
        car = ContractAbortRecord(param[u'ContractId'], param[u'ApplyRemark'], session['UserId'], session['DomainId'],
                                  datetime.now())
        c = Contract.query.filter_by(Id=param[u'ContractId']).one()
        if c.Status == ContractStatus.Last:
            return {'bSuccess': False}
...
@moudule.route('/checkTranVideo', methods=['GET', 'POST'])
@output_data
def checkTranVideo():
    try:
        video = File.query.filter_by(Status=2).one()
        if video:
            return video.VideoFile[len('http://123.57.157.64:8080/tmpfiles/'):]
        else:
            return 'none'
