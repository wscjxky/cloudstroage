...
def mobile_requirement(id):
    ...
    if request.Status >= RequirementStatus.Contracting:
        try:
            follower = RequirementFollower.query.filter_by(RequirementId=curId).all()
    ...
    if request.Status >= RequirementStatus.Reviewing:
        try:
            ...
            # mark
            try:
                mark = MarkPoint.query.filter(MarkPoint.AttachmentId == checkAttachment[0].Id).order_by(
                    MarkPoint.Time.asc()).all()

                for m in mark:
                    r = {'Id': m.Id, 'AttachmentId': m.AttachmentId, 'Time': m.Time.strftime("%H:%M:%S"),
                         'Content': m.Content}
                    marks.append(r)
            except Exception, e:
                pass
    ...
    if request.Status >= RequirementStatus.Retainage:
        videoAttachment = db.session.query(ContractAttachment).\
            filter(ContractAttachment.ContractId == request.ContractId).all()
    return render_template('mobile/mrequirement.html',MarkPoints=marks,IsService=curuser.Domain.IsService,req=request,follower=follower,rentOrder=rentOrder,checkAttachment=checkAttachment,restOrder=restOrder,videoAttachment=videoAttachment)
@weixing.route('/fuck', methods=['GET'])
def fuck():
...
@weixing.route('/pay_confirm', methods=['POST'])
@incoming_params
def pay_confirm():
    param = request.json_param
    if u'Id' in param:
