define(['jquery', 'template', 'storage', 'contract', 'search'], function ($, Template, Storage, Contract, Search) {
    Template.helper("RequirementStatusTranslator", function (Status) {
        var _status = {0: "未提交", 1: "已提交", 2: "合同建立", 3: "已完成"};
        return _status[Status];
    });
    Template.helper("ParseDate", function (date) {
        var d = new Date(date);
    ...
    function showTip(temp){
        $(temp).find("#requirement_title").popover({
            trigger: 'focus',
            title: '<red>请输入需求名称</red>',
            content:"完整、清晰的名称，有利于我们快速响应",
            html : true
        });
        $(temp).find("#requirement_detail").popover({
            trigger: 'focus',
            title: '<red>请输入需求的具体内容</red>',
            content:"详细、清晰的描述，有利于我们快速执行",
            html : true
        });
        $(temp).find("#requirement_long").popover({
            trigger: 'focus',
            title: '<red>请输入视频时长</red>',
            content:"时间长度有利于我们预估项目规模",
            html : true
        });
        $(temp).find("#requirement_format").popover({
            trigger: 'focus',
            title: '<red>请输入视频画质</red>',
            content:"若需要，请指定高清或4K",
            html : true
        });
        $(temp).find("#gbm").popover({
            trigger: 'focus',
            title: '<red>请输入背景音乐要求</red>',
            content:"无具体要求可不填",
            html : true
        });
        $(temp).find("#place").popover({
            trigger: 'focus',
            title: '<red>请输入拍摄要求</red>',
            content:"无具体要求可不填",
            html : true
        });
        $(temp).find("#refer").popover({
            trigger: 'focus',
            title: '<red>请输入视频样例</red>',
            content:"无具体要求可不填",
            html : true
        });
        $(temp).find("#subtitle").popover({
            trigger: 'focus',
            title: '<red>请输入字幕具体要求</red>',
            content:"无具体要求可不填",
            html : true
        });
        $(temp).find("#voice").popover({
            trigger: 'focus',
            title: '<red>请输入配音具体要求</red>',
            content:"无具体要求可不填",
            html : true
        });
        $(temp).find("#symbol").popover({
            trigger: 'focus',
            title: '<red>请输入角标具体要求</red>',
            content:"无具体要求可不填",
            html : true
        });
    }
    function checkInput(temp){
        if($(temp).find("#requirement_title").val()==''){
            alert("请输入需求标题");
            $(temp).find("#requirement_title").focus();
            return 0;
        }
        if($(temp).find("#requirement_detail").val()==''){
            alert("请输入需求内容");
            $(temp).find("#requirement_detail").focus();
            return 0;
        }
        if($(temp).find("#requirement_long").val()==''){
            alert("请输入视频时长");
            $(temp).find("#requirement_long").focus();
            return 0;
        }
        if($(temp).find("#requirement_format").val()==''){
            alert("请输入视频画质");
            $(temp).find("#requirement_format").focus();
            return 0;
        }
        if($(temp).find("#requirement_amount").val()==''){
            alert("请输入需求定价");
            $(temp).find("#requirement_amount").focus();
            return 0;
        }
        if($(temp).find(".deadline").val()==''){
            alert("请输入截止日期");
            $(temp).find(".deadline").focus();
            return 0;
        }
        var myDate = new Date();
        var m = $(temp).find(".deadline").val()[0]+$(temp).find(".deadline").val()[1];
        var d = $(temp).find(".deadline").val()[3]+$(temp).find(".deadline").val()[4];
        var y = $(temp).find(".deadline").val()[6]+$(temp).find(".deadline").val()[7]+$(temp).find(".deadline").val()[8]+$(temp).find(".deadline").val()[9];
...
//                }
    ...
    function openReq(id){
        handler.showRequirement(id);
    }

    var requirement_interface = {};
    var handler = new RequirementHandler();
    requirement_interface.pageRouter = {
        '/requirementPage': requirementPage,
        '/requirementPage([^/]*)': requirementPage,
        '/requirement/:id': openReq
    };
    return requirement_interface;
});