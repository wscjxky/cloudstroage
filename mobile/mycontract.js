...
$(".addMarkPoint").unbind('click').click(function (e) {
    ...
    console.log(content)
    addMarkPoint(ATTACHMENT_ID,content);
     });
$("#delMark").unbind('click').click(function (e) {
    delMarkPoint(MARK_ID);
});

$('.saveAttachment').unbind('click').click(function(e){
                        var id = $(e.currentTarget).attr("value");
                        var destObjId = 0;
...
function addMarkPoint(attachmentId, content){
                    $.ajax({
                        type: "POST",
...
function denyReqProducer(obj){
        FOLLOWE_ID=$(obj).prev().prev().val();
...
function acceptReqProducer(obj){
        FOLLOWE_ID=$(obj).prev().val();
...
$("#server-comfirm-btn").unbind('click').click(function (e) {
      if(!confirm('确认收款？'))
                    return false;
               $.ajax({
                    type: "POST",
                    url: "/mobile/pay_confirm",
                    dataType: "json",
                    contentType: "application/json",
                    data: JSON.stringify({
                        Id: REQ_ID,
...
$("#server-comfirm-btn1").unbind('click').click(function (e) {
      if(!confirm('确认收款？'))
             return false;
               $.ajax({
                    type: "POST",
...
var MARK_ID = $('#mark_id').attr('value');
